...
int Labyrinthe::LabyScript::RandomInt(int _min, int _max)
{
    return _min + rand() % (_max - _min + 1);
}

void Labyrinthe::LabyScript::Enclose(Vector<Vector<char>>& _grid)
{
    int h = _grid.size();
    int w = _grid[0].size();
...
void Labyrinthe::LabyScript::Recursive_division(Vector<Vector<char>>& _grid, int _xMin, int _xMax, int _yMin, int _yMax)
{
...
void Labyrinthe::LabyScript::Lobby(Vector<Vector<char>>& _grid, int _xMin, int _xMax, int _yMin, int _yMax)
{
...
void Labyrinthe::LabyScript::BetweenDivisionH(Vector<Vector<char>>& _grid, int _x, int _y, int _width)
{
    _grid[_x][_y] = ' ';
...
void Labyrinthe::LabyScript::BetweenDivisionV(Vector<Vector<char>>& _grid, int _x, int _y, int _height)
{
    _grid[_x][_y] = ' ';
...
void Labyrinthe::LabyScript::PierceWallH(Vector<Vector<char>>& _grid, int _y, int _xStart, int _xEnd, int _heightGrid)
{
...
void Labyrinthe::LabyScript::PierceWallV(Vector<Vector<char>>& _grid, int _x, int _yStart, int _yEnd, int _widthGrid)
{
    std::vector<int> positions;
...
void Labyrinthe::LabyScript::printGrid(Vector<Vector<char>>& _grid)
{
...
void Labyrinthe::LabyScript::bfs_check(Vector<Vector<char>>& grid)
{
...
void Labyrinthe::LabyScript::Laby3d(Vector<Vector<char>>& _grid)
{
    ...
    for (int x = 0; x < _grid.size(); x++)
    {
        for (int y = 0; y < _grid[0].size(); y++)
        {
            ...
            if (_grid[x][y] == 'D')
            {
                EntityId doorEntity = scene->world->CreateEntity();
				m_doors[doorCount] = doorEntity;
                TransformComponent& t = scene->world->AddComponent<TransformComponent>(doorEntity);
                MeshRenderer& m = scene->world->AddComponent<MeshRenderer>(doorEntity);
                m.geoId = RessourceManager::GetGeometryId("Cube");
                m.materialId = RessourceManager::GetMaterialId("DoorMaterial");
                t.local.SetScale(XMFLOAT3(cellSize, wallHeight, cellSize));
                float cx = x * cellSize - offsetX * cellSize;
                float cy = y * cellSize - offsetY * cellSize;
                t.local.SetPosition(XMFLOAT3(cx, wallHeight * 0.5f, cy));
                ColliderComponent& col = scene->world->AddComponent<ColliderComponent>(doorEntity);
                PhysicComponent& phys = scene->world->AddComponent<PhysicComponent>(doorEntity);
                phys.SetStatic();
				doorCount++;
            }
        }
...
void Labyrinthe::LabyScript::Awake()
{
    int m_widthGrid = 21; // doit être impair
    int m_heightGrid = 21; // doit être impair
	m_opened = false;
	m_timer = 5.0f;
	m_openDuration = 5.0f; // secondes
	m_started = false;

	CreateLabyrinthe(m_widthGrid, m_heightGrid);
}

void Labyrinthe::LabyScript::Update(float dt)
{
    if (m_started == true) 
    {
        m_timer += dt;
...
void Labyrinthe::LabyScript::CreateLabyrinthe(int _width, int _height)
{
...
void Labyrinthe::LabyScript::ReloadLabyrinthe(int _width, int _height)
{
	CreateLabyrinthe(_width, _height);
}

void Labyrinthe::LabyScript::DestroyLabyrinthe()
{
